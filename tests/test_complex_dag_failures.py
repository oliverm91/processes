...
def test_complex_dag_dual_independent_failures(tmp_path, monkeypatch) -> None:
...
"""
    template_path = tmp_path / "error_template.html"
    template_path.write_text(custom_template, encoding="utf-8")
    monkeypatch.setenv("PROCESSES_ERROR_TEMPLATE", str(template_path))
